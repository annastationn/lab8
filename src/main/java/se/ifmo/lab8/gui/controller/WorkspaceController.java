            ...
            try {
                ...
                for (int counter = 0; counter < Integer.parseInt(amount.getText()); counter++) {
                    ...
                    Address address = Address.builder()
                            ...
                            .id(isEdit ? organizationService.findById(selected.getId()).getOfficialAddress().getId() : null)
                            .build();
                    ...
                    // Check for existing address and update or create new
                    if (isEdit) {
                        Address existingAddress = organizationService.findById(selected.getId()).getOfficialAddress();
                        existingAddress.setZipCode(zipCode.getText());
                        address = existingAddress;
                    } else {
                        Optional<Address> existingAddress = addressService.findByZipCode(zipCode.getText());
                    ...
                    // Coordinates creation and assignment
                    Coordinates coordinates = new Coordinates();
                    coordinates.setX(coordinatesX.getText().equals("R") ? (long) (Math.random() * 300) : Long.valueOf(coordinatesX.getText()));
                    coordinates.setY(coordinatesY.getText().equals("R") ? (int) (float) (Math.random() * 300) : Integer.parseInt(coordinatesY.getText()));

                    if (isEdit) {
                        Coordinates existingCoordinates = organizationService.findById(selected.getId()).getCoordinates();
                        coordinates.setId(existingCoordinates.getId());
                    }
...
//                    organization.setAnnualTurnover(Float.valueOf(annualTurnover.getText()));
//                    organization.setOrganizationType(organizationTypeComboBox.getValue());
//                    organization.setOfficialAddress(Address.builder()
//                            .zipCode(zipCode.getText())
//                            .id(isEdit ? organizationService.findById(selected.getId()).getOfficialAddress().getId() : null).build());
//
//                    Coordinates coordinates = new Coordinates();
//                    coordinates.setX(coordinatesX.getText().equals("R") ? (long) (Math.random() * 300) : Long.valueOf(coordinatesX.getText()));
//                    coordinates.setY(coordinatesY.getText().equals("R") ? (int) (float) (Math.random() * 300) : Integer.parseInt(coordinatesY.getText()));
//
//                    if (isEdit) organization.setId(organizationService.findById(selected.getId()).getCoordinates().getId());
//
//                    organization.setCoordinates(coordinates);
//
//                    System.out.println(organization);
